...
1 do 2 i=1d0,j,k
  ...
  call ecf(q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,n,c1,c2,w,h)
  call spacedt(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,tempd)
  if(tempd.ge.1d0)then
  call orbitreform(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,inid,tempd)
  num=num+1d0
  tempd=inid
  else
  end if
  call fastlyindex(inid,tempd,fli,num)
2 continue
if(t.gt.1d1)then
j=20d0
else
...
d=((q1-sx)**2d0+(q2-sy)**2d0+(q3-sz)**2d0+(p1-px)**2d0+(p2-py)**2d0+(p3-pz)**2d0+(q4-cta)**2d0+(q5-ctb)**2d0+(p4-xia)**2d0+(p5-xib)**2d0)**(1d0/2d0)
return
end subroutine
