...
real*8 e,pe
real*8 h,endt,ht,tout1,tout2,tout3
real*8::t1=0d0,init=0d0,overt=0d0,t2=0d0,t3=0d0
real*8 inih,inil,temph,templ,r,fh,fl,psr,inis,temps,fs
real*8 inid,tempd,fli,el,num,tempdv,reorbit,inir,r0
integer i,j,k
integer::count=0d0
open(10,file=file1)
open(20,file=file2)
...
inir=r0
sx=inir
call initiala(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,n,c1,c2,w,e,inih,inil,ht,endt,j,k,inid)
call initialb(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,e,inid)
pause
tout1=0d0
tout2=0d0
call cpu_time(init)
...
2  tout1=tout1+ht
   !tout2=tout2+ht
  !call rks(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,n,c1,c2,w,ht)
  !call iss(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,n,c1,c2,w,ht)
  !call ems(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,n,c1,c2,w,ht)          
  call ecf(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,n,c1,c2,w,ht)
  call ecf(q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,n,c1,c2,w,ht)
  tempd=dsqrt((q1-sx)**2d0+(q2-sy)**2d0+(q3-sz)**2d0+(p1-px)**2d0+(p2-py)**2d0+(p3-pz)**2d0+(q4-cta)**2d0+(q5-ctb)**2d0+(p4-xia)**2d0+(p5-xib)**2d0)
      if(tempd.ge.1d0)then
      reorbit=reorbit+1d0
      q1=sx+inid*(q1-sx)/tempd
      q2=sy+inid*(q2-sy)/tempd
      q3=sz+inid*(q3-sz)/tempd
      q4=cta+inid*(q4-cta)/tempd
      q5=ctb+inid*(q5-ctb)/tempd
      p1=px+inid*(p1-px)/tempd
      p2=py+inid*(p2-py)/tempd
      p3=pz+inid*(p3-pz)/tempd
      p4=xia+inid*(p4-xia)/tempd
      p5=xib+inid*(p5-xib)/tempd
      tempd=inid
      else
      end if
if(tout1.lt.endt)goto 2
fli=reorbit*dlog10(1d0/inid)+dlog10(tempd/inid)
...
fli=0d0
reorbit=0d0
call hamiltonian(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,n,c1,c2,w,inih)
call angularmomentum(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,n,c1,c2,w,inil)
1 continue
inir=r0
e=e+0.02d0
!e=0d0
...
subroutine initialb(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,e,inid)
implicit none
real*8 sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,e,inid
q1=sx+inid
q2=sy
q3=sz
q4=cta
...
subroutine spacedt(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,d)
implicit none
real*8 sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,d
d=((q1-sx)**2d0+(q2-sy)**2d0+(q3-sz)**2d0+(p1-px)**2d0+(p2-py)**2d0+(p3-pz)**2d0+(q4-cta)**2d0+(q5-ctb)**2d0+(p4-xia)**2d0+(p5-xib)**2d0+1d-32)**(1d0/2d0)
return
end subroutine

subroutine orbitreform(sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,inid,tempd)
implicit none
real*8 sx,sy,sz,cta,ctb,px,py,pz,xia,xib,q1,q2,q3,q4,q5,p1,p2,p3,p4,p5,inid,tempd
q1=sx+inid*(q1-sx)/tempd
q2=sy+inid*(q2-sy)/tempd
q3=sz+inid*(q3-sz)/tempd
q4=cta+inid*(q4-cta)/tempd
